...
class ID666:
    def __init__(self, title, game, dumper, comments, date, playtime, fade, artist, disable_default_channel, emulator):
        self.title = title.strip(b'\x00').decode(u"UTF-8")
        self.game = game.strip(b'\x00').decode(u"UTF-8")
        self.dumper = dumper.strip(b'\x00').decode(u"UTF-8")
        self.comments = comments.strip(b'\x00').decode(u"UTF-8")
        try:
        ...
        self.artist = artist.strip(b'\x00').decode(u"UTF-8")
...
class SPC:
    def __init__(self, f: BinaryIO):
        ...
        if self.version_minor != 30:
            raise Exception(f'Invalid SPC file version: expected 30, got {self.version_minor}')
