...
NOTE_DURATIONS_V1  = [0xc0, 0x90, 0x60, 0x48, 0x40, 0x30, 0x24, 0x20, 0x18, 0x10, 0x0c, 0x08, 0x06, 0x04, 0x03]
NOTE_DURATIONS_V23 = [0xc0, 0x90, 0x60, 0x40, 0x48, 0x30, 0x20, 0x24, 0x18, 0x10, 0x0c, 0x08, 0x06, 0x04, 0x03]
NOTE_DURATIONS_V4  = [0xc0, 0x60, 0x40, 0x48, 0x30, 0x20, 0x24, 0x18, 0x10, 0x0c, 0x08, 0x06, 0x04, 0x03]
NOTES_COMMON = ['NOTE_C', 'NOTE_CS', 'NOTE_D', 'NOTE_E', 'NOTE_ES', 'NOTE_F', 'NOTE_FS', 'NOTE_G', 'NOTE_GS',
                'NOTE_A', 'NOTE_AS', 'NOTE_B']
NOTES_V12 = NOTES_COMMON + ['NOTE_REST', 'NOTE_TIE']
NOTES_V34 = NOTES_COMMON + ['NOTE_TIE', 'NOTE_REST']
OPCODES_V4_COMMON = {
    196: 'EVENT_VOLUME',
            ...
            if opcode >= 0xc3:
                ...
                match instruction['event']:
                    case 'EVENT_PITCH_SLIDE':
                        instruction['length'], instruction['semitones'] = unpack('BB', self.raw.read(2))
                    case 'EVENT_PAN_LFO_ON':
                        instruction['depth'], instruction['rate'] = unpack('BB', self.raw.read(2))
                    case 'EVENT_VIBRATO_ON' | 'EVENT_TREMOLO_ON':
                        instruction['delay'],  instruction['rate'], instruction['depth'] = unpack('BBB', self.raw.read(3))
                    case 'EVENT_ECHO_VOLUME_FADE' | 'EVENT_ECHO_FEEDBACK_FADE' | 'EVENT_TEMPO_FADE' | 'EVENT_VOLUME_FADE' | 'EVENT_PAN_FADE' | 'EVENT_ECHO_FIR_FADE':
                        instruction['length'], instruction['value'] = unpack('BB', self.raw.read(2))
                    case 'EVENT_GOTO':
                        (instruction['addr'],) = unpack('>H', self.raw.read(2))
                    case 'EVENT_LOOP_BREAK' | 'EVENT_CPU_CONTROLED_JUMP_V2':
                        instruction['value'], instruction['addr'] = unpack('>BH', self.raw.read(3))
                    case 'EVENT_VOLUME' | 'EVENT_VOLUME_ALT' | 'EVENT_MASTER_VOLUME' | 'EVENT_LOOP_START' | 'EVENT_PROGCHANGE' | \
                         'EVENT_TUNING' | 'EVENT_NOISE_FREQ' | 'EVENT_ADSR_AR' | 'EVENT_ADSR_DR' | 'EVENT_ADSR_SL' | \
                         'EVENT_ADSR_SR' | 'EVENT_JUMP_TO_SFX_LO' | 'EVENT_ONETIME_DURATION' | 'EVENT_PAN' | \
                         'EVENT_TRANSPOSE_ABS' | 'EVENT_TEMPO' | 'EVENT_ECHO_VOLUME' | 'EVENT_JUMP_TO_SFX_HI' | \
                         'EVENT_CPU_CONTROLED_SET_VALUE' | 'EVENT_OCTAVE' | 'EVENT_TRANSPOSE_REL':
